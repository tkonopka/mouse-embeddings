...
```{r fig.heterogeneity, eval=TRUE, echo=FALSE, fig.width=3.2, fig.height=3.3, fig.align="center", out.width="50%", fig.cap="\\textbf{Phenotype heterogeneity.} \\textbf{(A)} Summary of the multiplicity of models available for individual genes. The most represented genes are listed in the inset. \\textbf{(B)} Embeddings of mouse models with selected genes highlighted in color. Highlighted models all contain mutations in a single gene. Highlighted models are jittered to better display the number of models in each region. \\textbf{(C)} Proportion of genes for which the nearest-neighbors of a mouse model contain another model with the same gene knocked-out. The summary is stratified by the number of models available for a gene. Boxes represent 25\\%-75\\% intervals, whiskers represent 5\\%-95\\% intervals, middle lines represents the median. Dashed line indicates an expected level under a null hypothesis that neighbors are selected at random."}
...
```{r fig.diseases, eval=TRUE, echo=FALSE, fig.width=6.5, fig.height=4.3, fig.cap="\\textbf{Embedding of human disease in the mouse phenotypic space.} \\textbf{(A)} Projection of human diseases into an embedding of mouse models based on phenotype vectors. \\textbf{(B)} Analogous to (A), but with the underlying embedding produced based on semantic similarities of text-based descriptions. \\textbf{(C)} Summary of causal-gene extraction. Percentage captures the proportion of diseases for which one of 15 most-similar mouse models contains a mutation in the disease-associated gene. \\textbf{(D)} Summary of ORPHANET disease annotations in terms of phenotype ontology terms and causative genes. \\textbf{(E)} Projection of human diseases without any HP annotations into an embedding of mouse models based on text similarity. \\textbf{(F,G)} Examples of text-based disease descriptions along with two mouse models, selected manually from among the top five search hits."}
