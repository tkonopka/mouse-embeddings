...
Diseases with MP phenotypes: `r length(unique(disease_phenotypes$crossmap$id))`
Number of distinct HP phenotypes: `r length(unique(disease_phenotypes_hp$phenotype))`
Number of distinct MP phenotypes: `r length(unique(disease_phenotypes$crossmap$phenotype))`


```{r fig.models, eval=TRUE, echo=FALSE, fig.width=6.5, fig.height=4.9, fig.cap="\\textbf{Embeddings of mouse model phenotypes.} \\textbf{(A)} Embedding of mouse models based on vector representations of their phenotypes. Models are colored by the source of curated data. Labels and the rectangle point to selected models. \\textbf{(B)} Analogous to (A), but with the layout of mouse models produced using semantic similarities of text descriptions. \\textbf{(C)} Lists of phenotypes associated with individual mouse models highlighted in (A). Some lists are truncated for this visualization. All phenotype names match definitions from the ontology (abn.: abnormal, morph.: morphology, incr.: increased, decr.: decreased) \\textbf{(D)} A small region of the embedding in (A). \\textbf{(E)} Enrichment analysis comparing the phenotypes associated with mouse models in (D) against all other models outside the selected region. The significance threshold is Bonferroni-corrected p=0.05. The most significant phenotype is labeled."}
...
points_embedding(.emb2_diseases[!(id %in% .emb_diseases_ids)],
                 ...
                 Rcssclass="no_phen")
add_embedding_legend(legend_pos="topleft",
                     c(no_phen="Human disease (wo. HP annotations)",
                       other="mouse model"))
multipanelLabel(panel.labels[5])
#
