...
```{r supfig.models.covariates, eval=TRUE, echo=FALSE, fig.width=6.5, fig.height=6.5, fig.cap="\\textbf{Mouse model covariates.} Embeddings of mouse models based on non-binary vectors of phenotypes. Four panels differ by stratification strategy: \\textbf{(A)} phenotyping source; \\textbf{(B)} zygosity of gene knock-out; \\textbf{(C)} animal genetic background; \\textbf{(D)} number of phenotypes. \\textbf{(E)} Listing of the most frequent MP terms present among models with a single phenotype. Leading numbers indicate the number of models that are annotated with the stated term and no others."}
...
add_embedding_legend("topleft",
                     c(disease_hp="HP term appearing in a disease", mp="other HP term"))
multipanelLabel(panel.labels[3])
#
# panel with disease profiles (by coordinate averaging)
plot_embedding(.emb_hp[label=="hp"], detail=1,
...
points_embedding(.emb_hp[label=="disease"], detail=1,
                 Rcssclass="disease_hp")
add_embedding_legend("topleft",
                     c(disease_hp="disease profile", mp="HP term"))
multipanelLabel(panel.labels[4])
#
rm(.emb_mp, .emb_hp)
```
